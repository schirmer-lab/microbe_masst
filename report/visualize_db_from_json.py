...
### TODO bug fix: in visualization if translated name contains a "in: " replace it with empty string as it messes up the syntax

"""
Goal translate masst_tree.json to .nw files and create annotation file.
"""
...
# Function to convert JSON to Newick format
def json_to_newick(node, key):
    if "name" in node and node["name"] == "Blank" or node["name"] == "QC":
        return ""
    # Base case: Leaf node
    if "children" not in node or not node["children"]:
    #if "type" in node and node["type"] == "leaf":
        return f"{node[f'{key}'] if f'{key}' in node else '-1'}"
    
    # Recursive case: Internal node with children
    children_newick = ",".join(
        child_newick for child_newick in (json_to_newick(child, key) for child in node["children"]) if child_newick
    )
    return f"({children_newick}){node[f'{key}'] if f'{key}' in node else '-1'}"

# write nw file with taxIds
newick_str_taxId = json_to_newick(tree_data, "NCBI") + ";"

with open(f"{args.out_dir}/{args.prefix}_tree_from_json_taxID.nw", "w") as file:
    file.write(newick_str_taxId)

# write nw file with taxonomic names
newick_str_name = json_to_newick(tree_data, "name") + ";"

with open(f"{args.out_dir}/{args.prefix}_tree_from_json_name.nw", "w") as file:
    file.write(newick_str_name)
...
# create annotation
ncbi = NCBITaxa()
tree = Tree(f"{args.out_dir}/{args.prefix}_tree_from_json_taxID.nw")
create_annotation(args.out_dir, args.prefix, tree, ncbi, args.color_map)



# python visualize_db_from_json.py --json "../data/microbe_masst_tree.json" -o "output" -p "data_microbe_json"
"""
